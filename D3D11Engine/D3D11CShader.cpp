...
void D3D11CShader::UpdateBuffer(StringID name, const void* data, size_t size, std::source_location where) {
    if (const auto idx = GetInputIndex(name); idx != -1) {
        ...
        pool->BindCS(idx, pool->Allocate(data, size, where));
    }
}

void D3D11CShader::UpdateBuffer(UINT slot, const void* data, size_t size, std::source_location where) {
    const auto pool = reinterpret_cast<D3D11GraphicsEngineBase*>(Engine::GraphicsEngine)->GetConstantBufferPool();
    pool->BindCS(slot, pool->Allocate(data, size, where));
}
