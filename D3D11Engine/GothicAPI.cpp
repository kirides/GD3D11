...
void GothicAPI::LoadRendererWorldSettings( GothicRendererSettings& s ) {
    ...
    s.RainGlobalVelocity = XMFLOAT3(
        GetPrivateProfileFloatA( "General", "RainGlobalVelocityX", s.RainGlobalVelocity.x, ini.c_str() ),
        GetPrivateProfileFloatA( "General", "RainGlobalVelocityY", s.RainGlobalVelocity.y, ini.c_str() ),
        GetPrivateProfileFloatA( "General", "RainGlobalVelocityZ", s.RainGlobalVelocity.z, ini.c_str() )
    );
    ...
    s.RainFogColor = XMFLOAT3(
        GetPrivateProfileIntA( "General", "RainFogColorR", static_cast<int>(s.RainFogColor.x * 255.0f), ini.c_str() ) / 255.0f,
        GetPrivateProfileIntA( "General", "RainFogColorG", static_cast<int>(s.RainFogColor.y * 255.0f), ini.c_str() ) / 255.0f,
        GetPrivateProfileIntA( "General", "RainFogColorB", static_cast<int>(s.RainFogColor.z * 255.0f), ini.c_str() ) / 255.0f
    );
...
void GothicAPI::SaveRendererWorldSettings( const GothicRendererSettings& s ) {
    ...
    WritePrivateProfileStringA( "General", "RainGlobalVelocityX", std::to_string( s.RainGlobalVelocity.x ).c_str(), ini.c_str() );
    WritePrivateProfileStringA( "General", "RainGlobalVelocityY", std::to_string( s.RainGlobalVelocity.y ).c_str(), ini.c_str() );
    WritePrivateProfileStringA( "General", "RainGlobalVelocityZ", std::to_string( s.RainGlobalVelocity.z ).c_str(), ini.c_str() );
    ...
    WritePrivateProfileStringA( "General", "RainFogColorR", std::to_string( static_cast<int>(s.RainFogColor.x * 255.0f) ).c_str(), ini.c_str() );
    WritePrivateProfileStringA( "General", "RainFogColorG", std::to_string( static_cast<int>(s.RainFogColor.y * 255.0f) ).c_str(), ini.c_str() );
    WritePrivateProfileStringA( "General", "RainFogColorB", std::to_string( static_cast<int>(s.RainFogColor.z * 255.0f) ).c_str(), ini.c_str() );
...
void GothicAPI::OnVobMoved( zCVob* vob ) {
    auto checkMatrix = []( XMMATRIX& a, XMMATRIX& b ) -> bool {
        return memcmp( &a, &b, sizeof( XMMATRIX ) ) == 0;
    };
...
void GothicAPI::DrawSkeletalMeshVob( SkeletalVobInfo* vi, float distance, bool updateState ) {
    ...
    XMMATRIX matScale = XMMatrixScalingFromVector( model->GetModelScaleXM() );

    XMMATRIX world = vi->Vob->GetWorldMatrixXM() * matScale;
...
void GothicAPI::DrawParticleFX( zCVob* source, zCParticleFX* fx, ParticleFrameData& data ) {
    ...
    if ( pfx ) {
        ...
        switch ( emitter->GetVisAlphaFunc() ) {
        ...
        for ( p = pfx; p; p = p->Next ) {
            ...
            if ( emitter->GetVisTexAniIsLooping() != 2 ) { // 2 seems to be some magic case with sinus smoothing
                color.w = std::min( p->Alpha, 255.0f ) / 255.0f;
            } else {
                color.w = std::min( (zCParticleFX::SinSmooth( fabs( (p->Alpha - emitter->GetVisAlphaStart()) * emitter->GetAlphaDist() ) ) * p->Alpha) / 255.0f, 1.0f );
            }
            ...
            if ( RendererState.RendererSettings.EnableDynamicLighting && dist < visualFXDrawRadius ) {
                ...
                for ( int i = 0; i < leaf->LightVobList.NumInArray; i++ ) {
                    ...
                    if ( lightCameraDist + vob->GetLightRange() < visualFXDrawRadius ) {
                        ...
                        if ( !vi->VisibleInRenderPass && vob->IsEnabled() ) {
...
XRESULT GothicAPI::SaveMenuSettings( const std::string& file ) {
    ...
    std::string ini = std::string( NPath, len ).append( "\\" + file );

    LogInfo() << "Saving menu settings to " << ini;
    GothicRendererSettings& s = RendererState.RendererSettings;

    WritePrivateProfileStringA( "General", "WindowMode", std::to_string( s.WindowMode ).c_str(), ini.c_str() );
    ...
    */
    ...
    WritePrivateProfileStringA( "Display", "LimitLightIntensity", std::to_string( s.LimitLightIntensity ? TRUE : FALSE ).c_str(), ini.c_str() );
...
XRESULT GothicAPI::LoadMenuSettings( const std::string& file ) {
    ...
    if ( Toolbox::FileExists( ini ) ) {
        ...
        s.WindowMode = GetPrivateProfileIntA( "General", "WindowMode", defaultRendererSettings.WindowMode, ini.c_str() );
        ...
        */
        ...
        s.LimitLightIntensity = GetPrivateProfileBoolA( "Display", "LimitLightIntensity", false, ini );
...
void GothicAPI::CollectPolygonsInAABB( const zTBBox3D& bbox, zCPolygon**& polyList, int& numFound ) {
    ...
    CollectPolygonsInAABBRec( &BspLeafVobLists[bsptree->GetRootNode()], bbox, list );
