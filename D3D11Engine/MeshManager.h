...
public:
    ...
    std::shared_ptr<SkeletalMeshData> PutSkeletalData( zCSubMesh* subMesh, SkeletalMeshData* data ) {
        auto sharedData = std::shared_ptr<SkeletalMeshData>( data );
        m_skeletalMeshInfo[subMesh] = sharedData;
        return sharedData;
    }

    std::shared_ptr<SkeletalMeshData> GetSkeletalData( zCSubMesh* subMesh ) {
    ...
    std::shared_ptr<StaticMeshData> PutStaticData( zCSubMesh* subMesh, StaticMeshData* data ) {
        auto sharedData = std::shared_ptr<StaticMeshData>( data );
        m_staticMeshInfo[subMesh] = sharedData;
        return sharedData;
    }

    std::shared_ptr<StaticMeshData> GetStaticData( zCSubMesh* subMesh ) {
