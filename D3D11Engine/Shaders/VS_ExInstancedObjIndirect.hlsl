...
StructuredBuffer<VobInstanceInfo> instances : register(t1);
...
VS_OUTPUT VSMain( VS_INPUT Input )
{
    ...
    VobInstanceInfo instance = instances[Input.instanceID];

#if SHD_INFLUENCE
    
    if (instance.canBeAffectedByPlayer > 0)
    {
        // HERO MOVING BUSHES SHADER
        position += CalculatePlayerInfluence(playerPos, position, minHeight, maxHeight, instance.world);
    }
#endif
    
#if SHD_WIND
    
    if (instance.windStrenth > 0)
    {
        // WIND SHADER
        // Protect 0 height
        float heightRange = max(maxHeight - minHeight, 0.001);
        ...
        position += ApplyTreeWind(
            ...
            globalTime,
            instance.world,
            instance.windStrenth
        );
    }
#endif
    
    // Common processing for both cases
    float3 worldPos = mul(float4(position, 1.0), instance.world).xyz;
    
    // Calculate previous world position for motion vectors
    float3 prevWorldPos = mul(float4(position, 1.0), instance.prevWorld).xyz;

    Output.vPosition = mul(float4(worldPos, 1.0), frame.M_ViewProj);
    Output.vTexcoord = Input.vTex1;
    Output.vTexcoord2 = Input.vTex2;
    Output.vDiffuse = UnpackColor(instance.color);
    Output.vNormalVS = mul(Input.vNormal, mul((float3x3)instance.world, (float3x3)frame.M_View));
